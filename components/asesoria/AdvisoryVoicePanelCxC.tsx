...
import { Mic, Square, Info, RefreshCw, FileJson } from "lucide-react";
import { VoicePulse } from "@/components/voice/VoicePulse";
import { useVoiceAgent } from "@/hooks/useVoiceAgent";
import { Button } from "@/components/ui/button";

type CxCVoiceContextApiOk = {
  context: string;
  sourceFiles: string[];
  totalChars: number;
  truncated: boolean;
...
const FALLBACK_CONTEXT = JSON.stringify(
  {
    escenario: "Asesoría especializada · FIC CxC (resumen)",
    fuenteUnica: "Solo JSON digest VLM en /info2 — sin corpus CxC cargado aún.",
...
/**
 * Panel de voz CxC-only.
 * - Lee 3 archivos: Plataforma de Fondos (filtrada a CxC), Update FIC CxC marzo 2026, Prospecto FIC CxC.
 * - Inyecta el contexto inline (sin BrainBox, sin RAG).
 * - El agente está acotado por systemInstruction a hablar SOLO del fondo CxC.
 */
export function AdvisoryVoicePanelCxC() {
  const [cxcContext, setCxcContext] = useState<string | null>(null);
  ...
  const loadContext = useCallback(() => {
    ...
    fetch("/api/fund-advisory-cxc-voice-context")
      .then(async (r) => {
        if (!r.ok) {
          const err = await r.json().catch(() => ({}));
          throw new Error(typeof err?.error === "string" ? err.error : r.statusText);
        }
        return r.json() as Promise<CxCVoiceContextApiOk>;
      })
      .then((data) => {
        setCxcContext(data.context.trim() || FALLBACK_CONTEXT);
        setContextMeta({
        ...
        setLoadState("ready");
        console.log(
          `[asesoría-voz-cxc] Contexto CxC cargado: ${data.sourceFiles.length} archivos, ${data.totalChars.toLocaleString()} caracteres`,
        );
      })
      .catch(() => {
        setCxcContext(FALLBACK_CONTEXT);
        setContextMeta(null);
        setLoadState("error");
      });
  }, []);
  ...
  const voiceOpts = useMemo(
    () => ({
      ...
      fundAdvisoryContext: cxcContext ?? undefined,
      fundAdvisoryRag: false,
      fundAdvisoryScope: "cxc" as const,
      voiceName: "Aoede",
    }),
    [cxcContext],
  );
    ...
    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100 bg-gray-50/50">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Asesor FIC CxC por Voz</h3>
            <p className="text-xs text-gray-500 mt-0.5">
              Solo Fondo Abierto con Pacto de Permanencia CxC
            </p>
          </div>
          <div className="flex items-center gap-1.5 rounded-lg bg-white px-2.5 py-1.5 text-xs font-medium text-gray-700 shadow-sm ring-1 ring-gray-200">
            <FileJson className="w-3.5 h-3.5 text-[#4a7c59]" />
            <span>JSON · CxC</span>
          </div>
        </div>
      </div>
        ...
        <div className="flex justify-center">
          {error ? (
            ...
            <span className="rounded-full bg-gray-50 px-3 py-1 text-xs font-semibold text-gray-500 ring-1 ring-gray-200">
              Cargando base de conocimiento CxC…
            </span>
          ) : (
            <span className="rounded-full bg-gray-50 px-3 py-1 text-xs font-semibold text-gray-500 ring-1 ring-gray-200">
              Listo para iniciar
          ...
          <div className="mt-2">
            ...
            {showContextInfo && (
              <div className="mt-2 rounded-lg bg-gray-50 p-3 text-[10px] text-gray-600 space-y-1">
                <p>
                  <strong>Modo:</strong> Inline JSON (CxC-only, sin RAG)
                </p>
                <p>
                  <strong>Alcance:</strong> Fondo Abierto con Pacto de Permanencia CxC
                </p>
                <p>
